    ...
    arrange.on('node:mouseenter', ev => {
      if (ev.shapeName === 'action') {
        ...
        setActionPop({
          ...
          menu: [{key:`${ev.item.id}|open`, label:'打开'}, {key:`${ev.item.id}|close`, label:'关闭'}, {key:`${ev.item.id}|self`, label: ev.item.id}]
        })
      }
    })
  ...
  return (
    <div>
      ...
      <div className='flex flex-items-center relative'>
        ...
        <Dropdown rootClassName='node-action-dropdown' open={actionPop.open} placement="bottom" menu={{items: actionPop.menu.map(item => ({ key: item.key, label: item.label, onClick: handleMenuClick}))}} >
